...
function preload ()
{
    ...
    this.load.image('bg', 'images/escenario.png');
    this.load.image('block', 'images/tank.png');
}

function create ()
{
    this.cameras.main.setBounds(0, 0, 4096 * 2, 427).setOrigin(0,0);

    for (x = 0; x < 2; x++)
    {
        this.add.image(4096 * x, 0, 'bg').setOrigin(0);
    }

    cursors = this.input.keyboard.createCursorKeys();

    player = this.physics.add.image(150, 350, 'block').setOrigin(0,0).setAngle(0);
